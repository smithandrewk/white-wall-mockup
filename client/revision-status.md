...
- [ ] **Item 4 — crew add-on mechanics overhaul (DREW-80, IN PROGRESS).** Build plan: `client/comms/2026-08-25-drew-crew-mechanics-build-plan.md`. 4a remove placement questions + 4b April crew email (existing + dedicated 2nd) + 4c owner "Action required: SETUP/RESET ADD-ON for Session WWxxxx" email w/ dashboard deep-link → ONE booking PR (no info-loss gap). 4d dashboard crew-notes box (shown last, autosave) → dashboard PR. 4e draft-only personalized client email into Drew's drafts (LLM event-type rephrase) → infra-gated (`esc-item-4e-...`, mailbox access + LLM). No money/legal/arch escalation. On it.
