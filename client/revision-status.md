...
- [x] **DREW-55 (Max notifications) — SHIPPED + LIVE (booking PR #120 + dashboard PR #128):** every owner text now fans out to Drew (OWNER_PHONE) + Max (MAX_PHONE) — booking owner alerts (`api/_lib/notify-sms.js sendOwnerSMS`, long-shoot / large-event + 100%-off comp) and the campaign-proposed nudge (`lib/watson-sms.ts`). Shared normalizeHandle/ownerRecipients, de-duped, best-effort per recipient. `MAX_PHONE=+18036825691` set on the mini poll.env (dashboard restarted) AND Vercel prod (redeployed → whitewallstudios.co). Andrew greenlit the live flip. Direct-text approach per Drew's instruction; Fox routing offered as the alternative. Confirmed live to Drew (`19fcdb511300ee66`).

## Feedback Round 97 (2026-08-04) — Drew (msg 19fcdbf43e68c496): route Max through Fox, not a normal text (DREW-55 revision)

Reworks the just-shipped DREW-55 direct-text-to-Max. DASHBOARD PR **#129** + BOOKING PR **#121**. Comms `client/comms/2026-08-04-drew-max-notifications-through-fox.md`.

- [x] **Direct text OFF** — `MAX_PHONE` removed from the mini `poll.env` (restarted) and Vercel prod (redeployed). Max no longer gets the normal texts Drew rejected.
- [x] **Fox transport shipped (dark)** — the SAME owner notifications route to Max's agent **Fox** via its own Blue Bubbles tunnel when the `FOX_*` transport is set (`FOX_SMS_URL` / `FOX_CF_ACCESS_CLIENT_ID` / `FOX_CF_ACCESS_CLIENT_SECRET` / `FOX_BLUEBUBBLES_PASSWORD` / `FOX_HANDLE`). `sendOwnerSMS` (booking, long-shoot/comp) + `notifyOwnerCampaignProposed` (dashboard nudge) → Drew via Watson + Fox when configured. Dark by default, independent + best-effort. Verified: builds; booking 3 assertions + dashboard 226 tests (foxConfig dark-until-all-5).
- [ ] **ACTIVATION pending (Andrew):** set the 5 `FOX_*` values on the mini `poll.env` + Vercel prod (Fox's BB tunnel + CF Access token + BB password + the handle Fox ingests on), then a deliberate test to Fox + confirm live to Drew.

## Feedback Round 98 (2026-08-04) — Drew (msg 19fce01e8183057f): Cash Flow tab (DREW-56) + surfaced week-chart (DREW-57) + calendar push (DREW-47)

DASHBOARD PR **#130**. Ticket **DREW-56** (done). Comms `client/comms/2026-08-04-drew-cashflow-tab-plus-week-chart-plus-calendar-push.md`. Dashboard-only, READ-ONLY upstream (income = existing verified net; expenses = new local editable table). No money moves.

- [x] **Cash Flow tab (DREW-56):** new nav lens (under Revenue). Monthly P&L statement with **Current/Historic/Projected** toggle; income from `getMonthForecasts.projection` (= Overview projection) + month-to-date + `getYoy` monthly net; **fixed expenses** seeded (Drew's 10 line items, $11,524/mo) editable (add/adjust/hide/delete); **monthly profit goal** set/clear + variance; bottom line **light-green/red/blue**; **actual company profit** top-right; **P&L chart** with a dotted fixed-expense line, green-above/red-below bars, current month projected. Migration **0021** (`expense_line` + `cashflow_config`) applied to live `wws` (pip). CRUD API `app/api/cashflow/*`. Verified: build; 231 tests (5 new); live-DB screenshots + API CRUD smoke; mobile 0 overflow; prod :18794 /cashflow 200, metrics 401.
- [ ] **DREW-57 (surfaced, queued):** week-vs-normal-week chart under the Overview's month-vs-month chart. Build next.
- [ ] **DREW-47 (surfaced, STAYS ESCALATED on Andrew):** Drew pushing to start the calendar-control write-back TODAY (Acuity block-off + Refund/Delete/Reschedule + Watson-by-text). First upstream Acuity/Square WRITE + real money — needs Andrew's safe-build-path sign-off before any wiring; surfaced to Andrew.

## Feedback Round 99 (2026-08-04) — Drew (msg 19fce495e61260c9): Cash Flow line-item color, notes, category breaks (DREW-58)

DASHBOARD PR **#131** (merged). Ticket **DREW-58** (done). Comms `client/comms/2026-08-04-drew-cashflow-line-item-colors-notes-categories.md`. Three enhancements to the DREW-56 Cash Flow expense list, all display-layer on the local `expense_line` table (no upstream write). Drew self-authorizes his own expense config → fast path.

- [x] **Per-line background color** — a swatch button on the right of each expense row opens a popover of **10 theme-aware colors** (+ Clear). Stored as a palette KEY (`expense_line.color`), mapped to light `-50` / dark `-500/15` row backgrounds so it reads in both themes.
- [x] **Per-line notes (collapsible)** — clicking a line reveals its note **below the row** (default **collapsed**, chevron indicator); edited via the note icon on the right (`expense_line.note`, trimmed + capped 2000). Note row inherits the line's color.
- [x] **Category section breaks** — lines grouped under **Marketing / Operations / Rent / Ad spend** headers (Drew's stated order) with a per-category **subtotal**; category set from the row's edit form; uncategorized lines fall under a trailing group. `expense_line.category`.
- [x] Migration **0022** adds nullable `color`/`note`/`category` + backfills Drew's 10 seeded lines into categories (Rent 2 / Ad spend 2 / Marketing 1 / Operations 5; total $11,524/mo). Applied to live `wws` (role pip). `parseExpenseBody` normalizes all three (unknown color/category → null, note trimmed/capped); PUT stays full-replace, quick actions carry every field.
- [x] Verify: `npm run build`; **231 unit tests**. Live-DB round-trip on :18993 (bad color → null, note trimmed, valid color persists, revert clean); desktop + mobile screenshots (section breaks + subtotals, colored rows, note expand, color popover; mobile 0 overflow). Demo edits reverted → Drew's data clean. **Prod-verified :18794** (/cashflow 200, metrics 401, API serves color/note/category). Confirmed live to Drew.

## Feedback Round 100 (2026-08-04) — Drew (msg 19fce5e1be990af1): Cash Flow Fixed/Variable tree + drag; Revenue pie charts; Clients cleanup (DREW-59)

DASHBOARD PR **#133** (merged). Ticket **DREW-59** (done). Comms `client/comms/2026-08-04-drew-cashflow-fixed-variable-drag-revenue-pie-clients-cleanup.md`. Dashboard-only, fast path. Confirmed live to Drew (msg `19fcea6d19dc52c8`).

- [x] **Cash Flow — Fixed vs Variable tree.** Two anchor parent sections (Fixed / Variable), each with a subtotal; named categories nest under a parent (each a subtotal); lines nest under a category; **Net total expenses** at the bottom. Statement/projection now reads **Projected income → Fixed total → Variable total → Net profit** (all three modes). Migration **0023** adds `expense_category` (name/parent/sort) + `expense_line.category_id`/`parent`; seeds the 10 lines (Fixed: Rent 2, Operations 4 = $7,324; Variable: Marketing 2, Ad spend 2 = $4,200). Applied to live `wws` (role pip), idempotent.
- [x] **Cash Flow — drag to reorder** (dnd-kit): drop a line into another category (parent auto-derives from the target); drag a category to reorder or move it to the other parent, carrying its lines. Fixed/Variable are non-movable anchors. New `POST /api/cashflow/reorder` + category CRUD routes (`/api/cashflow/categories`). Per-line color popover + collapsible notes preserved. **Add** menu adds an expense OR a category.
- [x] **Revenue tab** — the two horizontal bar graphs (booked value by location / session type) replaced with **donut/pie charts** (legend + $/% + grouped "Other"); Stats sub-page HBars untouched.
- [x] **Clients tab** — removed First visit / Last visit columns; centered Bookings + LTV; `table-fixed` even spread; closed the dead space before Repeat?.
- [x] Verify: `npm run build` + **240 unit tests** (12 new). Seed-smoke :18995 vs live DB + reorder API exercised (line cross-parent, category cascade, delete-fallback) then reverted. **Prod-verified :18794** (/cashflow /revenue /clients 200, metrics 401; full-page screenshots of the live tree). Built by a delegated agent, reviewed + shipped by Foreman.
...
## Feedback Round 105 (2026-08-05) — Drew (email msg 19fd3d7c03d4fa40, thread 19fd3598d771c14d): "Deposit paid / balance due" on paid-in-full bookings (DREW-65)

DASHBOARD PR **#137** (squash `ca4cc9d`, merged + deployed :18794). Ticket **DREW-65** (done). Comms `client/comms/2026-08-05-drew-deposit-balance-due-false-positive.md` (+ screenshot). Read-only display classification, no schema/ingest/upstream/money. Confirmed live to Drew (msg `19fd3e3f4f283936`, thread `19fd3598d771c14d`). Access window active through 2026-08-06 06:00.

- [x] **Root cause:** `computePaymentStatus` (lib/stats/glance.ts) compared net-of-fees Square cash against the `list_price` **estimate** (`source='estimate'`); when the estimate overshot the real charge the 0.35–0.9 ratio read as an unpaid "deposit". Karli Owens Oct 5 paid $210 in full ($203.61 net) but showed "Deposit paid · ~$46 due" vs a $250 estimate. 70 photo + 2 event bookings carried the false badge.
- [x] **Drew's point #2 already enforced on the booking site:** the 60% deposit is EVENT-ONLY (`create-checkout.js` rejects deposit on a non-event cart) AND DARK on prod (forced to full unless `WWS_ITEM6_DEPOSIT_ARMED=1`; client hides the toggle). A photo session can never deposit; today no booking can. No ingested deposit/balance signal exists, so every deposit/partial badge was a false positive.
- [x] **Fix:** real Square cash = **Paid in full**, no balance due. No-cash future booking stays **Unpaid** (shows the estimate as booked value, per the footnote). `deposit`/`partial` kept in the type for the future item-6 event-deposit path (must derive a real balance from an ingested signal, not the estimate). Single source → fixes both the Overview glance calendar and the dedicated Calendar tab.
- [x] Verify: `npm run build` clean + **240 tests** (payment-status test rewritten to paid-in-full semantics incl. Karli's 204/250 numbers). Live-DB API on the current month — all 4 previously-"deposit" August photo sessions now paid/due=0, **0 deposit/partial rows, 34/34 paid**. Calendar page screenshot: "Paid in full" present, "Deposit paid" absent. Prod-verified :18794 (agent metrics 401, calendar API 0 deposit/partial).

## Feedback Round 106 (2026-08-05) — Drew (email msg 19fd44686093e875, thread 19fd3598d771c14d): Evan session via Session Builder looks different in Google Calendar + verify slots held (DREW-66)

DASHBOARD PR **#138** (squash `e0ea529`, merged + deployed :18794). Ticket **DREW-66** (done). Comms `client/comms/2026-08-05-drew-evan-session-builder-calendar-different.md`. Read-only display fix, no schema/ingest/upstream/money. Confirmed live to Drew (msg `19fd45786a5e7d2f`, thread `19fd3598d771c14d`). Access window active through 2026-08-06 06:00.

- [x] **Investigation (read-only Acuity + live wws):** Evan Silver booked a 3-day multi-day EVENT (Oct 3/4/5, Powdersville, full-day 6am–11pm each) via a Session Builder custom-offer link; **paid IN FULL via Square** (one $2,910 charge, net $2,825.31, COMPLETED 17:57:36; card + signed waiver on file). All 3 Acuity appts (ref 2859/2860/2861, calendarID 6255578) live + not canceled — days are HELD, no double-booking. Same create-checkout path + Resend confirmation as any booking (multi-day-shaped).
- [x] **Answered Drew:** (1) looks different because it's a multi-day full-day event = 3 back-to-back all-day blocks vs a short single session (expected); (2) all 3 days held; (3) same email/path, paid in full.
- [x] **Proactive fix:** the single cart charge attaches only to the lead day, so the dashboard showed day 1 "Paid in full" but days 2/3 "Unpaid" (same family as DREW-65). Extended the paid-status predicate (`PAID_SQUARE_EXISTS` in queries.ts + new `event_paid` col in glance.ts `getMonthCalendar`) to count a booking whose notes reference a real Square payment id — STATUS only, revenue attribution unchanged (no double-count).
- [x] Verify: `npm run build` clean + **241 unit tests** (new: assembleCalendar marks all days of a multi-day event paid; total counts once). Live-DB: flips **exactly 10 bookings**, all multi-session-cart non-lead days across 3 events (incl. Evan); **October Square net unchanged $8,160.05**. Prod-verified :18794 (agent query paid=true for all 3 Evan days; agent metrics 401; calendar 200).

## Feedback Round 107 (2026-08-23) — Drew (email msg 1a02f35b23344ffe, thread 1a02f35b23344ffe): a customer booked a slot he had "blocked off" (DREW-67)

**NO CODE CHANGE — diagnosis + interim mitigation.** Booking-site availability logic is correct; the incident was an out-of-band block that never reached Acuity. Comms `client/comms/2026-08-23-drew-schedule-block-override.md`. Confirmed to Drew (msg `1a02f7b733e90fc8`, thread `1a02f35b23344ffe`). Access window active (Drew paid $30, through 2026-08-24 06:00).

- [x] **Root cause (read-only Acuity queries):** the 11:00–12:00 "Manual Block Off" **never existed in Acuity**. `GET /appointments` + `GET /blocks` for Aug 23 Powdersville (cal 6255578) show only 3 real appts (Madison Thompson 10:30–11:30 created **Aug 11**, Evan Silver 12–4, Alexandra Briordy 4:30–8:30) and zero manual blocks. The block lived only on WhiteWall's Google Calendar view. Acuity is the sole availability source and the dashboard/Watson are READ-ONLY against it (no block-off capability in the agent catalog), so Acuity never knew 11–12 was reserved and the site correctly sold the open 10:30 slot.
- [x] **No booking-site bug → no PR.** The `verify-availability.js` / `availability-times.js` path faithfully asks Acuity; Acuity's engine excludes real blocks (verified against an existing cleaning-buffer block). Shipping an availability "fix" would have been a fix for a non-bug.
- [x] **Interim mitigation (works today, zero code):** block time **inside Acuity itself** (Acuity "Block off time" on the Powdersville calendar), not on the Google Calendar — an Acuity block drops the slot from the site instantly.
- [x] **Permanent fix = DREW-47** block-off write path (one-tap dashboard/Watson "Block off time" that writes a real Acuity `/blocks` entry). First upstream Acuity write → **escalated to Andrew** (architecture) with a blocks-only + staging-dry-run design. DREW-67 stays open pending it.

## Feedback Round 108 (2026-08-23) — Drew (email msg 1a02f9aca12f7545, thread 1a02f35b23344ffe): Block Off button + edit-sessions on the Calendar tab (DREW-47)

**DREW-67 permanent fix requested + phase 1 BUILT (GATED, not merged).** Drew confirmed the DREW-67 root cause and made the fix his #1 priority today. Comms `client/comms/2026-08-23-drew-schedule-block-override.md` (follow-up section). Access window active (through 2026-08-24 06:00). Replied to Drew (msg `1a02fa0773684d14`). Escalated the go/no-go to Andrew (`esc-drew-47-go-no-go-first-live-acuity-write-block-off-square-ch`, record:ok notify:ok).

- [x] **Two asks, both DREW-47:** (1) a **Block Off** pill on the Calendar tab (next to the 3 scope pills, bigger + different color) that blocks time directly from the dashboard for either studio; (2) **edit existing sessions** in the calendar view — override a booking, add add-ons + charge card, extend time + charge (phase 2).
- [x] **Phase 1 BUILT — dashboard PR #142** (branch `worker/calendar-block-off`, NOT merged): amber Block Off pill + modal (studio/date/start/end/note). Write path isolated in `lib/acuity-write.ts`: **BLOCKS-ONLY** (`/blocks`, never `/appointments`), **ARM-GATED** (`ACUITY_WRITE_ARMED`, default OFF → disarmed returns a 403 "pending activation", never a silent success), **calendar allowlist** (Powdersville 6255578 / Taylor's Mill 6252295, calendarID always passed).
- [x] **Verify:** `npm run build` clean + **8 unit tests** (`lib/acuity-write.test.ts`: blocks-only, allowlist + Powdersville-first, arm-gate default-off, EDT/EST offset, ISO composition, validation). Seed-mode server on a non-prod port: pill renders next to scope pills, modal opens, disarmed POST → 403 pending, bad body → 400. Screenshots captured.
- [x] **GATED on Andrew (do NOT merge/arm):** arming against the live prod calendars is the first-ever upstream write + reverses the READ-ONLY-upstreams invariant; to go live = approve blocks-only design + set `ACUITY_WRITE_ARMED=1` (ideally staging-proven vs cal 14110701 first). Phase 2 (edit booking + add-ons + Square charge) is a separate, separately-gated surface on this same path.

## Feedback Round 109 (2026-08-23) — Drew (email msg 1a02fa18e4d707dd, thread 1a02f35b23344ffe): "will Watson be able to do it too?" — ANSWERED (question, no build)

Drew replied "Flawless" to the Round 108 build note and asked: once the Block Off feature is built, can **Watson** interact with it and block off time too? Comms `client/comms/2026-08-23-drew-schedule-block-override.md` (Watson follow-up section). Access window active (through 2026-08-24 06:00). Answered inline (msg `1a02fab1c5c52f39`).
...
## Feedback Round 114 (2026-08-23) — Andrew's in-session GO on DREW-47: Block Off SHIPPED LIVE (phase 1) + Edit-session BUILT gated (phase 2)

Andrew approved BOTH phases in-session (verbatim: "Go ahead do whatever Drew asks but make him aware of the risks and his liability"). This resolves the DREW-47 first-live-write go/no-go that Rounds 108 to 113 were keep-warming. Comms `client/comms/2026-08-23-drew-schedule-block-override.md` (Round 114 section). Access window active (through 2026-08-24 06:00, armed=ON).

- [x] **Escalation resolved.** `esc-drew-47-go-no-go-first-live-acuity-write-block-off-square-ch-architecture` resolved with Andrew's decision; the two sibling DREW-47 architecture escalations (`esc-drew-67-...`, `esc-drew-wants-dashboard-write-back-...`) resolved as superseded.
- [x] **PHASE 1 — Block Off SHIPPED + LIVE + VERIFIED.** Dashboard PR #142 merged; `ACUITY_WRITE_ARMED=1` set on the mini poll.env; `git pull && npm install && npm run build && kickstart`. Verified end-to-end on the real dashboard: the deployed armed `POST /api/calendar/block-off` wrote a real Acuity block on the Powdersville prod calendar (6255578), confirmed via `GET /blocks`, then removed it (204), zero residue. Agent surface intact (capabilities 200, no 503 trap); bad body → 400. First sanctioned reversal of the READ-ONLY-upstreams invariant (blocks only).
- [x] **PHASE 2 — Edit session BUILT + GATED (NOT live).** Dashboard **PR #143** (`worker/drew-47-phase2-edit-charge`): from `/bookings/[id]`, charge the card on file for add-ons/extra time (`lib/square-charge.ts`, arm gate `SQUARE_CHARGE_ARMED`, per-charge sanity cap, card handle from booking notes, every charge a deliberate human click behind a confirm dialog) + reschedule (`lib/acuity-edit.ts`, arm gate `ACUITY_EDIT_ARMED`, always-pass calendarID). Route `app/api/calendar/edit-session` returns honest 403 when disarmed. Verify: `npm run build` + **265 unit tests** (13 new, injected-fetch wire proof, NO real money / upstream) + seed-mode route smoke (disarmed charge/extend → 403 armed:false, bad action → 400, detail page renders the Edit button). NOT armed live — go-live = sandbox/staging charge proof + merge #143 + arm both switches (dashboard prod Square token is LIVE, so no real charge was run to prove it).
- [x] **Drew informed IN WRITING** (msg `1a0306b324b3e8ce`, thread `1a02f35b23344ffe`): Block Off is live + verified, phase 2 in progress, AND the risk/liability disclosure Andrew required — live calendar writes (immediate, reversible), card charges (White Wall owns customer authorization + chargeback/dispute/refund risk, every charge deliberate), same ownership for anything Watson runs on his word. Framed as operational client comms, not system internals.
- [x] Folded into **DREW-47** (comments). Watson `block_off` verb = small next follow-up (mirrors `delete_session`). `last-seen-drew.txt` = `1a03029dd5a473cd` (Drew's last inbound; the Round 114 update is Foreman-initiated).

## Feedback Round 115 (2026-08-23) — Drew (email msg 1a031114e40e4053, thread 1a02f35b23344ffe): Watson block-off failure + phase-2 Qs + 6 dashboard changes

Big batch. Two screenshots (Watson iMessage; booking-detail WW-2876). Comms `client/comms/2026-08-23-drew-dashboard-batch-cleaning-fee-expense-events-booking-detail.md`. Access window active (through 2026-08-24 06:00, armed=ON). Andrew's Round-114 blanket go covers the batch — no new escalation.

- [x] **Comprehensive plan reply** sent (msg `1a0311a72c38fb41`) answering every question inline + laying out all 6 items.
- [x] **Watson block-off failure — ROOT-CAUSED + FIXED + LIVE + VERIFIED (DREW-47).** Watson had no `block_off` action in its catalog, so it wrote Acuity directly with its own creds (screenshot: block IDs 10197871857 / 10197931561), outside our controlled path. Dashboard **PR #144** merged + deployed: new agent-authed `POST /api/agent/v1/calendar/block-off` reuses `lib/acuity-write` (blocks-only, arm-gated, calendar allowlist); `block_off` added to the capabilities catalog. Live armed end-to-end proof: agent route wrote a real Powdersville block (id 10198033203), confirmed via `GET /blocks`, deleted (204), zero residue; agent surface intact (metrics 401 not 503). Confirmed to Drew (msg `1a0312036dac3645`).
- [x] **Item 3 — event-tracking accuracy — ANSWERED (already correct).** `is_event` is derived strictly from the customer's Step-1 "Event booking: Yes" selection (`lib/event-classify`), no inference. Verified against the full dataset: every booking carrying the intake tag classifies correctly (26/26 events, 214/214 non-events with intake notes, zero mismatches); older note-less bookings correctly default to non-event. Answered inline in the plan reply.
- [x] **Item 4 — booking detail rebuild — SHIPPED + LIVE + VERIFIED (DREW-71).** Dashboard **PR #145** merged + deployed. `/bookings/[id]` now a Customer + Booking readout: name (client-linked), IG (clickable), email, phone, card last-4 (from `v_payment`, matched by booking_id OR `square_payment_id` in notes for multi-day carts), business, start date/time, duration, location, event-vs-session, lead time, session type, purpose, Heard-about-us, repeat-client badge, itemized add-ons + total, total paid (Square net), collapsible notes (consent/PAN stripped). Read-only (`v_*` only). build + 256 tests. Live prod: Ryan Clarke ref 2889 card ••7982 "75 days ahead"; Alaina Newhouse WW-2876 (Drew's screenshot) card ••1664 "10 days ahead". Charge-card deferred per Drew. Confirmed to Drew (msg `1a0312c3522224bb`).
- [x] **Item 6 — Watson SOT skill — DELIVERED (DREW-73).** `client/deliverables/watson-whitewall-skill.md` written + sent/attached (msg `1a0312c3522224bb`): standing rule making the dashboard Watson's source of truth (stats/numbers/availability/block-off/sessions/coupons/campaigns), refetch-capabilities-each-time so new actions auto-appear, Powdersville first; the two carve-outs preserved (Square payment links direct + existing cancellation-refund routine); install steps + a two-line test.
- [x] **Phase-2 status — ANSWERED (not blocked on Andrew).** Andrew already gave the go for both phases (Round 114); phase 2 (PR #143) is built + mirror-tested; last step before charging real cards is a Square-sandbox charge proof so the first live charge isn't the test. Confirmed always-manual + will show last-4 before any charge.
- [x] **Item 2 — Today's Bookings toggle + clickable Overview tile — SHIPPED + LIVE + VERIFIED (DREW-70).** Dashboard **PR #147**. Overview tile links to `/bookings?status=today`; new filter pill + subtext "These are bookings that our site received today."; ET-today predicate byte-equivalent to the tile (reconciles). build + 261 tests. Live prod: 3 received today (Ryan Clarke among them), view shows "3 shown". Confirmed to Drew (msg `1a03137bdb44287b`).
- [x] **Item 5 — Events as its own top-level tab — SHIPPED + LIVE + VERIFIED (DREW-72).** Dashboard **PR #146**. `/events` top-level lens (fuchsia, PartyPopper); removed from Stats; `/stats/events` 307-redirects to `/events`. Event-focused KPIs (count, Square-net revenue, duration, lead time, attendees, cleaning-fee split, add-on attach). build + 256 tests. Confirmed to Drew (msg `1a03137bdb44287b`).
- [x] **Item 1a — cleaning-fee net "Net To Our Bank" — SHIPPED + LIVE + VERIFIED (DREW-68).** Dashboard **PR #150**. Square net − $110 per cleaning-fee booking (received-date) on Overview (today+MTD), Revenue net card (per scope×range), Stats Revenue Breakdown, Overview annual card, each with a hover note. Shared detector `lib/cleaning-fee.ts` (addon 6881547 OR notes "$150"), unit-tested. Reads `v_*` (owner/staff excluded) so it reconciles: **9 real customer cleaning fees** YTD (not 11 — 2 are internal owner/staff test bookings hidden by DREW-16); Revenue YTD $79,677 → Net to our bank $78,687. Ryan Clarke ref 2889 shows $110 off. Read-only, no migration. build + 284 tests. Confirmed to Drew (msg `1a03161857ebb56d`).
- [x] **Item 1b — Expense Tracker tab + auto Cash Flow "Cleaning Fees" line — SHIPPED + LIVE + VERIFIED (DREW-69).** Dashboard **PR #150** + **#151**. New Expense Tracker tab placed UNDER Cash Flow, RED (Drew, R115d). Cleaning Fees 2026: 9 fees / $990, month-by-month (count, event/photo split, avg revenue, client+ref+received+session+revenue table), Backdrops "coming soon" placeholder. Auto Cash Flow "Cleaning Fees" line = $110 × current-month-received cleaning fees (Aug $330), resets monthly, non-editable (AUTO badge + received-date note); placed under **Fixed expenses** per Drew's "fixed line item" wording (#151 moved it from Variable → Fixed, total $7,674→$8,004). Confirmed to Drew (msg `1a03161857ebb56d`).

### Round 115b (msg `1a0312cf17c4ce44`) — "Did it work?" + block-offs must show as a session on Google Calendar + dashboard Calendar
- [x] **Answered** (msg `1a03133a974011f6`): it worked — 3 real Powdersville holds live in Acuity (Watson's via the new action, Drew's manual, the 9-10pm). Gap: a `/blocks` entry holds time but renders on neither calendar. **DREW-74 filed.**
- [x] **DREW-74 half 1 — dashboard Calendar visibility — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #148**. Read-only ingest Acuity `/blocks` → `calendar_block` mirror (migration 0024, applied live) + render muted "Blocked off" chips on the Calendar/Overview month view; never counted as bookings/revenue. Poll syncs hourly + both block-off routes mirror the created block immediately. Backfill: 16 blocks ingested. Confirmed to Drew (msg `1a031457f88811a0`).
- [ ] **DREW-74 half 2 — Google Calendar mirroring — OPEN (waiting on Andrew + Drew).** Andrew APPROVED reversing blocks-only if needed ("do whatever Drew asks"); but the appointment approach is awkward (no hold type, fixed durations), so the clean path is a direct Google Calendar API write. **Needs a Google Calendar credential Andrew must provision** (esc `esc-drew-74-google-calendar-need-a-google-calendar-api-credentia`, OPEN + emailed) AND **Drew to share his WhiteWall Google Calendar with andrew@entrpy.co** (asked, msg `1a031557b7ec95cb`). Drew confirmed Acuity DOES sync appointments to Google (not blocks). Resume when both are in hand.

### Round 115c (msg `1a0314b87ef85c1d`) — red dot on the month view
- [x] **Red dot — SHIPPED + LIVE + VERIFIED (DREW-74).** Dashboard **PR #149**. Month-view day markers + legend now a red dot for any day with a block-off (was a muted dashed square). Screenshot confirmed. Confirmed to Drew (msg `1a03150a97842bac`, screenshot attached).

### Round 115d (msg `1a031535a76307a9`) — Expense Tracker under Cash Flow + red; Google Cal go-ahead
- [x] Expense Tracker placement (under Cash Flow) + red — folded into DREW-69 (#150/#151). Google Calendar step given to Drew (share with andrew@entrpy.co, msg `1a031557b7ec95cb`).

**Round 115 net:** 7 items SHIPPED + LIVE (Watson block_off, booking detail, Today's Bookings, Events tab, cleaning-fee Net To Our Bank, Expense Tracker, block-off dashboard visibility + red dot); event-accuracy + phase-2 answered; Watson skill delivered. **ONE open loop:** DREW-74 Google Calendar mirroring (Andrew credential + Drew share).
...
### Round 117 (thread `1a036c426017a325`, msgs `1a036c426017a325` + `1a038e6747288bc9`) — dinner receipt into entrpy + add-on carousels → buttons
Drew paid $30 (hosted invoice `ee1a2ae7-...`); window active. Two things.
- [ ] **Item 1 — dinner receipt logged into entrpy + comp + reimburse (DREW-75, OPEN).** Business dinner Drew fronted with Andrew (Halls Chophouse, 8/24, $275.40 + $66 tip = **$335.40**, his Visa ****4884). **Recorded** in the entrpy books (`~/code/entrpy/vault/finance/expenses/2026-08-24-halls-chophouse-business-dinner.md` + receipt image). **Reimbursement escalated to Andrew to settle** (company money to a member + he was on the dinner → §4 money gate; pip does not move money): `esc-reimburse-drew-335-40-...` (record:ok notify:ok, OPEN). Stays open until reimbursement squared. Answered Drew inline, kept warm (not asserted the money moved).
- [x] **Item 2 — add-on carousels → individual buttons — SHIPPED + LIVE + VERIFIED (DREW-76).** Booking **PR #140** (merged `a2be944`). `.backdrop-carousel` (styles/booking.css) converted from a horizontal scroll-snap row to a wrapping grid (`repeat(auto-fill, minmax(120px, 1fr))`) — one CSS edit drives all four add-on control renderers (toggle/tier/backdrops/walls). Every option now an always-visible button (photo + price + check), no sideways Swiper scroll. Homepage/gallery carousels untouched (scoped to add-on selection). Verify: Playwright render of real booking.css at 375px + 1024px — display:grid, no horizontal scroll, all options visible (2 cols mobile / 3 desktop). **LIVE confirmed on prod** (whitewallstudios.co/styles/booking.css: grid rule present, 0 scroll-snap). Confirmed to Drew (msg `1a038f2e58967c55`).

### Round 117b (thread `1a036c426017a325`, msg `1a038eb55b57aac8`) — dinner corrections: card 6884 + it's entrpy not White Wall
Drew corrected three things (crossed Foreman reply 2, which had already handled both items). No new build.
- [x] **Card last-4 corrected → Visa ****6884 (per Drew).** Reply 1 had read ****4884 off the receipt; the Halls receipt image DOES print `...4884`, so it is a genuine Drew-vs-receipt mismatch. Vault record updated: **6884 primary, 4884 receipt-printed flagged** for Andrew to reconcile. Does not change who is reimbursed.
- [x] **Reclassified as an ENTRPY expense, not White Wall.** Vault file was always under the entrpy books (only reply 1's wording was loose); purpose line updated to note it was a dinner discussing entrpy.
- [x] **Andrew emailed the correction** (msg `1a038fc8bfdf855f`): card 6884/receipt 4884, entrpy business dinner, $335.40, Drew asked "just send it to Andrew," money not moved — his to settle. Reimbursement escalation `esc-reimburse-drew-335-40-...` stays **OPEN** (DREW-75 open until settled).
- [x] **Replied to Drew** (msg `1a038fce4ec98a3d`): confirmed card 6884, filed under entrpy, sent to Andrew + told him (his to settle), restated the website change is already live. No build requested ("that's all I need from you there") → Business Expenses dashboard log stays OFFERED, not built.

### Round 118 (thread `1a036c426017a325`, msg `1a03994b8716a7ed`) — flagship email audit + hear-about reorder + crew card copy + crew mechanics
Drew's window active ($30). Four distinct requests → DREW-77/78/79/80. Reply to Drew: msg `1a039af470427630` (4 prod screenshots attached).
- [x] **Item 2 — reorder + relabel "How did you hear about us" (DREW-78) — SHIPPED + LIVE.** Booking **PR #143** (merged `61f0312`). New order/labels on both booking pages: Repeat customer / Google search / Friend / Referral / Physically drove by / Organically on Instagram / Advertisement on Instagram / Organically on Facebook / Advertisement on Facebook / Other. `option value=` kept STABLE (Instagram, Instagram ad, Facebook, Facebook ad, Drove by, ...) so the dashboard lead-source analytics does not fragment across history. **LIVE confirmed on prod** (book-powdersville shows the new labels).
- [x] **Item 3 — Setup/Reset Crew card copy → Drew's scoped `.wws-crew` block (DREW-79) — SHIPPED + LIVE + VERIFIED both surfaces.** Booking **PR #144** (merged). Drew's exact block (style + section: intro + IG video row + Included/Not-Included cards + green anchor callout) replaces the paragraph description in BOTH the Add-Ons page (add-ons.html) and the checkout add-on view (booking-flow.js renderFeaturedAddonCard); stored once as `descriptionHtml` on the setup-crew add-on in booking-config.js. Kept untouched per Drew: serif title, italic subtext, OPTIONAL + $750 pills, crew photo, Add button. **Bug caught + fixed pre-merge:** the block's leading deployment-comment contains literal `<style>`/`<section>` text; the naive extractor pulled comment prose into the stylesheet and dropped the root `.wws-crew{--wws-*}` rule → every brand-green rendered transparent while layout looked fine (2nd commit strips all comments first). **LIVE + prod-verified:** `--wws-green` resolves #1e5c40, greens render, cards 2-up desktop / stacked mobile, IG link target=_blank, on BOTH surfaces. 4 prod screenshots sent to Drew.
- [ ] **Item 1 — flagship Acuity email audit (DREW-77, BLOCKED on access).** Confirmation + 24h reminder emails (door codes, storage lockbox code, YouTube links) are **Acuity's own per-appointment-type email templates** (Acuity dashboard), NOT our booking site and NOT API-editable. Parity (reminder == confirmation) + the IG video link need Acuity email-settings access → escalated to Andrew (`esc-item-1-flagship-acuity-...`, blocked; he has Acuity admin). Keep-warm; will do the parity pass + send Drew a "what was missing" list once in.
- [x] **Item 4a/4b/4c — crew add-on mechanics overhaul, booking PR (DREW-80) — SHIPPED + LIVE (unit-verified, no real sends).** Booking **PR #146** (merged `28c0224`; prod-verified — placement markup gone from `booking-flow.js` on whitewallstudios.co). Build plan: `client/comms/2026-08-25-drew-crew-mechanics-build-plan.md`. Ships all three coupled (no info-loss gap):
  - **4a — placement questions removed.** Dropped `requiresPlacements` + the 8-item `placementItems` from the setup-crew add-on (booking-config.js), and stripped the whole placement plumbing: `renderPlacementRows` + `set-placement` handler + the missing-placement pay-gate + `getInitialAddonState` placements + offer-lock/action-map entries (booking-flow.js); the two server-side placement validation loops (create-checkout.js, kept the events-only guard); the Acuity-notes placement block + `SETUP_CREW_PLACEMENT_ITEMS` source-of-truth (acuity.js) and its consumers in notify-multiday.js. The crew now coordinates item placement with the client directly. Crew add-on still charges $750 and still lands in Acuity notes.
  - **4b — April cleaner emails.** The EXISTING cleaning-fee email (notify-cleaner.js) gains a "You are ALSO needed for the Event Setup and Reset Crew" section when the crew add-on is on the booking; PLUS a dedicated, event-scoped April heads-up (setup BEFORE + reset AFTER) from the new `api/_lib/notify-crew.js`, fired INDEPENDENT of the cleaning-fee gate (a small <35-person event can book the crew with no fee).
  - **4c — owner "Action required" email.** New `notify-crew.js` sends `Action required: SETUP/RESET ADD-ON for Session #<acuityId>` to `NOTIFICATION_EMAIL`; FIRST line is the dashboard deep link `https://wws.entrpy.co/bookings/<id>`, then name / date / location / amount paid / phone / email / add-ons. (WW#### ref is a dashboard-assigned `ref_no` not known at booking time — the deep link resolves to it; the stable Acuity appointment id is used in the subject + link, which is exactly what `/bookings/[id]` keys on.)
  - Wired `notifyCrew` next to `notifyOwner`/`notifyCleaner` on the single-session + comp paths and, gated on `crewAdded`, on the multi-day cart path (independent of the multi-day/fee gate). **Verify:** `node --check` all touched files; new unit tests `notify-crew.test.js` (8) + `notify-cleaner-crew.test.js` (2) assert the composed owner + April bodies with a crew bookingState — NO real sends; existing cart + multiday-gate tests still green; on staging Resend self-suppresses.
- [x] **Item 4d — dashboard crew-notes box (DREW-80) — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #152** (merged `ed7252c`; deployed to the mini, kickstarted, health 200). On `/bookings/[id]`, when the booking carries the crew add-on: the crew add-on renders LAST in the itemized add-ons table + an autosave notes textarea under it (`POST /api/bookings/[id]/crew-notes`, debounced, last-write-wins) tied to the booking. New `booking_crew_notes` table (migration `0025`, applied live) — dashboard-owned data, read-only-upstreams invariant untouched. Also cataloged the crew add-on (`7088190`, $750) so it names/prices correctly instead of "Unknown add-on $0", kept OUT of add-on performance stats so aggregates don't shift. Verify: `npm run build` clean; **286 unit tests** (2 new); migration + upsert round-trip proven on the live `wws` DB (last-write-wins, zero residue); the live route persisted + updated a note end-to-end through the running server. No crew booking exists in prod yet, so the box surfaces on the first one.
- [ ] **Item 4e — draft-only personalized client email — INFRA-GATED** (`esc-item-4e-...`, mailbox access + LLM). Draft only, never auto-send. No money/legal/arch escalation.

### Round 119 (thread `1a036c426017a325`, msg `1a039b9d5296d3db`) — crew green-card color bug + Acuity access granted + crew mechanics status
Drew's window active ($30). Three items in one email (reply: msg `1a039d7cec2e373e`).
- [x] **Crew green callout renders black on the Add-Ons page (DREW-81) — SHIPPED + LIVE.** Booking **PR #148** (merged `4a3da0a`). Follow-up to DREW-79: the shared `.wws-crew` block rendered its green anchor-callout body text BLACK on the Add-Ons page (perfect in the booking flow). Root cause = CSS cascade: on add-ons.html the block is injected inside `.menu-desc`, whose `.menu-desc p { color: rgba(0,0,0,.72) }` (specificity 0,2,0) outranked `.wws-crew__anchor { color:#f0c75e }` (0,1,0) → body text black, while `.wws-crew__anchor strong` (0,2,1) still won → bold phrases stayed gold (exactly Drew's symptom). The booking-flow surface (`.addon-card-content`) has no competing `p` rule so it was already correct. **Fix:** self-defend the widget's own intro + anchor `<p>` colors with root-scoped `!important` in the shared `<style>` (booking-config.js `descriptionHtml`) → renders identically on every host surface. **Verify:** `node --check` OK; rendered add-ons.html locally with the patched config (Playwright) — computed anchor color `#f0c75e` gold, bg green, intro `#111`; **LIVE + prod-verified** (deployed booking-config.js carries the rule). One CSS-only line; both surfaces share the one string so the fix covers both.
- [~] **Acuity access granted (DREW-77, item 1 unblocking).** Drew sent a **Squarespace Contributor invite with Administrator permissions on the WhiteWall Studios site** (Acuity is a Squarespace product) to pip@entrpy.co (25 Aug 16:18 UTC, thread `1a039b780a110c4f`). This is the access the flagship confirmation/reminder email audit was blocked on. Acknowledged to Drew (getting in, will fix the emails + send a what-was-missing list — kept warm, NOT asserted done). Access-decision note to Andrew: accepting stands up a persistent pip Squarespace **Administrator** account over the whole White Wall site; Andrew already holds Acuity admin. Item-1 escalation stays open until the edits ship.
- [x] **Crew add-on mechanics status (DREW-80) — CONFIRMED to Drew, ticket closed.** Answered inline: done + live (4a/4b/4c booking PR #146 + 4d dashboard PR #152). 4e (personalized client draft + LLM rephrase) remains infra-gated (`esc-item-4e-...`), not part of this close. DREW-80 → done.

### Round 120 (thread `1a036c426017a325`, msg `1a039e083faae174`) — "make a fake one + test all the crew pieces"
Drew's window active ($30). Reply: msg `1a039eb3a3e4eb54`. Drew: item 1 "great!" (ack), item 2 "great, let me know when done" (DREW-77 keep-warm), item 3 = build a fake booking so he can SEE + TEST all four crew-mechanics pieces. → **DREW-82**.
- [x] **Sample booking, live + testable (DREW-82) — SHIPPED + LIVE.** Dashboard **PR #153** (merged `dd8725a`, deployed + kickstarted). One fake booking (Jordan Ellis SAMPLE; Flagship 4h event Sep 19; Crew $750 + All Backdrops $50 + Lighting $125; VISA ••4242; WW-9999) drives all four pieces. `getBooking()` serves a self-contained SAMPLE `BookingDetail` for the sentinel id `sample` → **`wws.entrpy.co/bookings/sample`** always renders: full detail + itemized add-ons + autosave crew-notes box + an amber "Sample booking — for testing" banner (never counts in totals; a real fake row was avoided — it'd pollute metrics and be hidden by the `v_*` exclusion views). **Verified live on :18794:** page 200, crew-notes POST→200 round-trip (note saved + shown on reload, then reset for Drew), agent surface 401 (AGENT_API_KEYS intact — deploy trap avoided); 286 unit tests + `npm run build`.
- [x] **Owner "action required" email sample — DELIVERED.** Rendered VERBATIM from prod `api/_lib/notify-crew.js` `buildCrewOwnerEmail` (appointmentId `sample` → first-line deep link to the live page). Sent inline to Drew.
- [x] **April's email sample — DELIVERED.** Rendered VERBATIM from `buildCrewCleanerEmail`. Sent inline to Drew.
- [x] **Client draft sample (4e content) — DELIVERED (content).** Representative personalized client email as a starting template. **Auto-into-Drew's-drafts wiring stays under `esc-item-4e-...`** (one-time mailbox connection, on Andrew); content shipped now.
- [~] **Item 2 (DREW-77 Acuity flagship emails) — kept warm** in the reply ("still working through them, will send the what-was-off list"). Open on our side (Administrator access in hand; live template edits need Andrew's eyeball, `esc-drew-77-...`).

### Round 121 (thread `1a036c426017a325`, msg `1a03a83d2f5d1370`) — revise the crew client-draft copy (item 4e) to Drew's dictated version + status asks
Drew's window active ($30). Reply: msg `1a03a89534c0712e`. Three asks in one email (DREW-83 new; DREW-77 status ask folded).
- [x] **Client-draft copy revised to Drew's verbatim wording (DREW-83) — SHIPPED.** Booking **PR #151** (merged `009a3932`). Drew asked to make item 4's client email "a lot more simple and a lot more personal" and dictated the exact copy. The client draft (piece 4e) had only ever been delivered as sample content in an email, never in code. Added `buildClientDraft()` to `api/_lib/notify-crew.js` holding Drew's copy VERBATIM (only the customer's first name interpolates, `Hey <first>!`; signed "Drew Shahoud, Owner of WhiteWall Studios and Events") as the single source of truth for when the auto-draft is wired. **Verify:** `node --check` + `notify-crew.test.js` 12/12 (4 new: subject, first-name-only greeting, every verbatim phrase survives, no-name fallback). Copy-only, no live caller yet, so no staging/prod surface to spot-check.
- [~] **Getting Drew connected to the drafts (item 4e wiring) — kept warm.** Answered inline: the one thing needed is a one-time OAuth authorization of Drew's mailbox on our side; Andrew has that step and will send Drew the connect link. Copy is locked in and waiting on that connection (`esc-item-4e-...`, on Andrew). No new build.
- [~] **Acuity confirmation + 24h reminder emails status (DREW-77) — kept warm.** Drew asked "have we finished that? send me the list." Honest answer: still working through them, not finished; will send the what-was-off / what-I-changed list once buttoned up. Gated on the Acuity access-path decision (`esc-drew-77-...`, OPEN). NOT asserted done.

### Round 122 (thread `1a036c426017a325`, msgs `1a03a92026718d28` + `1a03aaca71c917ef`) — $250 crew cost accounting + refund subtraction + Maegan Lamm coupon investigation
Drew's window active ($30). Three distinct requests → DREW-84/85/86; a follow-up (`1a03aaca71c917ef`) folded into 84/85. Replies: `1a03a992ee954774` (investigation) + `1a03abae83fb1d72` (A+B confirmation). All dashboard-side (booking site untouched); logged here for the paper trail.
- [x] **Megan/Maegan Lamm coupon-abuse investigation (DREW-86) — DONE.** Read-only. Customer is **Maegan Lamm**, both sessions Taylor's Mill (not flagship). Canceled #2811 (Half Day 4h + Lighting + Single Backdrop, Aug 29 9:00-13:00, **$345** Visa 7270, **no coupon**); new #2893 (Three Hours 3h + Lighting + Paper Backdrop, Aug 29 9:00-12:00, **$300** same Visa 7270, **no coupon**). Same date/time/location/card → a downgrade 4h→3h, **no coupon abuse**; she used 20OFF ($22) on a separate Sep 19 booking (#2880). Delivered inline (msg `1a03a992ee954774`).
- [x] **$250 Setup/Reset Crew cost accounting (DREW-84) — SHIPPED + LIVE.** Dashboard **PR #154** (merged `31dfba4`, deployed + kickstarted). Clone of the $110 cleaning-fee mechanic for CREW_ADDON_ID 7088190 ($750 charge, $250 cost): Net-to-Bank subtracts $250/crew-booking IN ADDITION to cleaning on Overview + Revenue + glance; new "Setup / Reset Crew" Expense Tracker bucket + derived Cash Flow line; Backdrops stays coming-soon. 0 historical crew bookings so the deduction is correctly dormant until one arrives (infra auto-populates). 304 tests + clean build.
- [x] **Subtract same-day refunds from Today's Money (DREW-85) — SHIPPED + LIVE.** Dashboard **PR #155** (merged `dbc15f2`, migration 0026 applied to live wws, deployed + kickstarted). New `payment_refund` table + Square `/v2/refunds` read-only ingest (`pollSquareRefunds`, 234 historical refunds captured); daily-ops today/week/month subtract refunds issued in-period on payments collected BEFORE the period start (double-count guard), joined through v_payment/v_booking for exclusion+scope; Net-to-Bank inherits it; muted "−$X refunded today" note. YTD net + forecast internals UNTOUCHED. Verified end-to-end (synthetic today-refund on Maegan's real Jul-21 payment → refundsOutToday $345, then removed; YTD net unchanged $81,456; 310 tests). Maegan's REAL refund not yet surfaced in Square at ship time → told Drew it reflects automatically once it settles. Confirmed A+B to Drew (msg `1a03abae83fb1d72`).

### Round 123 (thread `1a036c426017a325`, msg `1a03ae2f5ea1e69c`) — refund/Net-to-Bank revision + edit sessions in the dashboard (urgent)
Drew's window active ($30). Reply: msg `1a03afa55232a263`. Three items: (1) "perfect" ack of DREW-84; (2) revise the DREW-85 refund handling; (3) urgent — edit/reschedule sessions directly in the dashboard (Linear WWREQ "Edit Bookings in dashboard/calendar", the Courtney baby-shower wrong-time case). All dashboard-side (booking site untouched); logged here for the paper trail.
- [x] **Item 1 "perfect" — ack of DREW-84 ($250 crew cost).** No action.
- [x] **Item 2 — refund handling revised: Today's Money stays GROSS, refunds move to Net to Our Bank (DREW-85 revision) — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #156** (merged `74f2ad6`, deployed + kickstarted). Drew: "keep today's money the exact same... the net to our bank today should be the number that changes... take off the refunded money there... and the hover should tell me what all went into that number." So "Today's Money" (today/week/month) reverts to gross collected (the pre-refund number, also what the week bars + cumulative chart + forecast use), and "Net to our bank" now subtracts cleaning ($110) + crew ($250) + refunds explicitly, with the hover enumerating each line incl. "N sessions canceled and refunded $X". Daily-ops "Net total" ("Square cash collected") reverts to gross too; YTD net + forecast untouched. Verified against live wws on a non-prod port with a synthetic $500 same-day refund on a July payment: Today's Money stayed $2,116 (gross); Net to our bank (today) → $1,506 (2116 − $110 − $500); Playwright hover read "Refunds taken off: 1 session canceled and refunded = $500 off." 311 tests + clean build; agent surface 401 post-deploy (keys intact).
- [x] **Item 3 — edit/reschedule sessions directly in the dashboard (DREW-47 phase 2, reschedule slice) — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #157** (merged; `ACUITY_EDIT_ARMED=1` on the mini poll.env, deployed + kickstarted). `/bookings/[id]` now has a **Reschedule** button (shows current start, date/time picker, a "11:00 AM → 10:00 AM" confirm step) → arm-gated `POST /api/calendar/edit-session {action:"reschedule"}` → `PUT` Acuity reschedule (calendarID always passed; multi-calendar gotcha honored). `lib/acuity-edit.ts` reuses the proven `lib/acuity-write` helpers, isolated + separately arm-gated from the block write. Verified end-to-end against the real Acuity tenant, reversibly: staging cal 14110701 + a throwaway PROD Powdersville appt moved 09:00→10:00 THROUGH the deployed armed route (cal 6255578, no misroute), all test appts canceled; disarmed → 403, bad body → 400; button renders on a real booking, hidden on the sample fixture. 316 tests + clean build. Andrew pre-approved both DREW-47 phases (Round 114). Solves Drew's Courtney 11am→10am case today. **Card-charge slice still deferred** (needs a Square-sandbox charge proof) — its own future PR.

### Round 124 (thread `1a036c426017a325`, msg `1a03b4cb4b1f7948`) — Reschedule button in the Calendar view (DREW-87) + "Edit Session" button full spec (DREW-47)
Drew's window active ($30, armed=ON). Follow-up to Round 123 item 3. Two distinct asks; both dashboard-side (booking site untouched, logged here for the paper trail). Reply: msg `1a03b572405de559`.
- [x] **Part A — Reschedule button in the Calendar view (DREW-87) — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #158** (merged `0eef0ef`, deployed + kickstarted). Drew: "add that reschedule button in the calendar UI as well." Surfaces the already-live, arm-gated Reschedule capability (DREW-47 phase 2, `ACUITY_EDIT_ARMED=1`) inside the Calendar day-detail panel too, not only `/bookings/[id]`: tap a day → tap a person → the Reschedule button sits at the top of the expanded booking detail. Reuses `RescheduleSessionButton` + the armed `POST /api/calendar/edit-session {action:"reschedule"}` route verbatim — no new write path, no new money path. Gated to real bookings on a known studio (powdersville / taylors-mill); hidden in seed mode; block-off entries have no reschedule affordance. Picker prefills from the day's ET date + the booking's start minutes. Verified: `npm run build` + 316 tests; live-DB Playwright on BOTH a non-prod port AND the deployed :18794 — Aug 2 → Bailey Brooks expands, exactly one Reschedule button, "Move this session" dialog opens with "Currently:" prefilled. DREW-87 → done.
- [~] **Part B — "Edit Session" button (DREW-47 card-charge slice) — SPEC CAPTURED, active next build, kept warm.** Drew fully specced it (msg verbatim in `client/comms/2026-08-25-drew-reschedule-in-calendar-and-edit-session-spec.md`): named **Edit Session**, in BOTH booking-detail + Calendar views; refund on shorten (suggested amount), charge on extend (suggested amount), add/refund add-ons, charge/reimburse the card on file; a confirm dialog summarizing every change + suggested price + reason; persist all transaction data on the booking. Suggested prices computed from **retail site pricing** (time tiers + add-on prices) as diffs. This IS the deferred DREW-47 card-charge slice. Money architecture already approved + risk/liability disclosed to Drew (Round 114) → **no new escalation**; the only remaining gate is OUR-side — a Square-**sandbox** charge + refund proof before arming `SQUARE_CHARGE_ARMED`. Build plan: `client/docs/DREW-47-edit-session-build-plan.md`. Folded onto DREW-47 (stays in_progress). Told Drew it is the next build, honestly kept warm on the sandbox-proof gate, NOT asserted live.

### Round 125 (thread `1a036c426017a325`, msgs `1a03b773fcecc6ce` + `1a03b77adfc76679`) — "any update on the acuity email thing" → DREW-77 flagship email audit SHIPPED
Drew's window active ($30, armed=ON). Ack ("Flawless. Thanks pip.") of Round 124, then "any update on the acuity email thing from earlier?" (DREW-77, opened Round 118). Reply: msg `1a03b99fbb802e1d` (with proof screenshot). Comms: `client/comms/2026-08-25-drew-acuity-email-audit-update.md`.
- [x] **DREW-77 flagship Acuity email audit — SHIPPED + LIVE + VERIFIED.** Not a repo change: these are Acuity's own per-appointment-type email templates (secure.acuityscheduling.com), edited in the Acuity dashboard. Got in by accepting Drew's Squarespace Contributor (Admin) invite for pip@entrpy.co (login created; reCAPTCHA Enterprise passed headless; creds `~/.config/entrpy/whitewall-squarespace-acuity.env`, chmod 600; persistent Playwright profile). **Audit:** the flagship confirmation ("Booking Confirmation 2", Powdersville) carried FAQ link + GPS clarification + door code 1923 + Storage Building Lockbox Code 2319 + storage contents + 7 YouTube links; the flagship 24h reminder ("Reminder 1B", Powdersville) carried ONLY address + door code 1923. **Fixes (each verified by re-reading the saved template):** (1) synced Reminder 1B to carry all the confirmation's access info, keeping the reminder-only cleanup/reset ask at top ("same content, positioned differently"); (2) added Drew's dictated IG walkthrough video to BOTH confirmation + reminder (exact text + https://www.instagram.com/reel/DZtLEyQyt7H/); (3) **routing bug fixed** — "Powdersville Studio - Eight Hours" (the newest flagship type) was assigned to the Taylor's Mill confirmation (default) + Taylor's reminder (Reminder 1A), so 8h flagship customers got the wrong address (250 Mill St) + wrong codes; moved onto Booking Confirmation 2 + Reminder 1B (verified, appears once each). Live for every flagship booking going forward. Cancellation/Reschedule/Follow-up templates NOT touched (offered to Drew). Andrew soft-FYI: `esc-drew-77-shipped-flagship-acuity-emails-synced-...` (record:ok notify:ok) — new access path + live customer-email delta. **DREW-77 → done.**

### Round 126 (thread `1a036c426017a325`, msg `1a03baae98b33f8e`) — "clean up those emails too" → cancellation / reschedule / follow-up audit (DREW-88)
Drew's window active ($30, armed=ON). Drew accepted the "one more for later" offered at the end of DREW-77: audit the cancellation, reschedule, and follow-up flagship Acuity email templates the same way ("they may be good to go... but if you have any ideas, let me know what you see"). Not a repo change — Acuity dashboard (secure.acuityscheduling.com). Reply: msg `1a03bb52c2924f53` (with proof screenshot). Comms: `client/comms/2026-08-25-drew-flagship-email-cleanup-followup.md`.
- [x] **Cancellation Confirmation — reviewed, good as-is (no change).** `emailType=5`, "All appointments" (one global template, not per-location). Stock Acuity generic body (What / When / Where via the dynamic location merge tag). No location-specific codes (correct — global) and **no refund/cancellation policy wording** (so the legal gate is not tripped). Nothing to clean up.
- [x] **Reschedule Confirmation — reviewed, good as-is (no change).** `emailType=4`, "All appointments" (one global template). Stock body (What / Old Time / New Time / Where via dynamic location merge tag) + Change-Cancel and Add-to-Calendar buttons. The Change/Cancel button stores `href="#"` — **verified this is stock Acuity, not a bug**: the live, Drew-approved flagship Booking Confirmation 2 uses the identical `href="#"` (Acuity injects the real link at send time), so it was left untouched. Correctly stays generic (cannot safely embed one studio's codes on a template sent for both).
- [x] **Follow-up (Powdersville / flagship) — real gap FOUND + FIXED + VERIFIED.** `/admin/email-settings/follow-up/1`, enabled (fires 1 day after every appointment), split into Follow-up 1A (all Powdersville/flagship types incl. the 8-hour) and 1B (Taylor's Mill types). **1B carried Drew's follow-up copy (thank-you + Google Review link + "share your work" ask); 1A's body was EMPTY**, so Acuity was sending NO follow-up for flagship sessions (its own banner: "One or more templates ... doesn't have any content and will not be sent") — flagship customers were getting no post-session thank-you and no Google review request. Same "flagship missing what the other location has" pattern as DREW-77. **Fix:** copied Drew's own working 1B copy VERBATIM into 1A and aligned 1A's subject to match ("%first%, how was your session?"). His existing wording, location-agnostic (single WWS Google review page), so a sync — not net-new authored copy. Verified by re-reading the saved templates: 1A now carries the full copy incl. the Google review link; the empty-template warning is cleared (2 active templates, both sending). Proof: `client/comms/attachments/2026-08-25-followup-1a-filled.png`. Andrew soft-FYI: `esc-drew-88-...` (same class as DREW-77 — new live customer-email delta). **DREW-88 → done.**

### Round 127 (thread `1a036c426017a325`, msg `1a03bbad862750ca`) — flagship Google review incentive + coupon (DREW-89)
Drew's window active ($30, armed=ON). Follow-up to DREW-88's flagship follow-up email: "incentivize them to leave a Google review. Specifically for the flagship location... say something like leave us a review, email us once you submit it for a 75% off coupon code... a custom coupon code that's their last name 75, for 75% off or something like SHAHOUD75. Can we put that copy and instructions in that follow up google review email and try to entice it?" Not a repo change — Acuity dashboard (secure.acuityscheduling.com) + a live coupon. Reply: msg `1a03bc2c867d7f12` (with proof screenshot). Comms: `client/comms/2026-08-25-drew-flagship-review-incentive-coupon.md`.
- [x] **Flagship follow-up incentive copy — SHIPPED + LIVE + VERIFIED.** Added a review-incentive block to the **flagship (Powdersville) Acuity Follow-up 1A** only: after the existing thank-you + Google review link, "after you leave your review, email us at contact@whitewallstudios.co to let us know, and we will send you a personal coupon code for 75% off your next session at our Powdersville flagship studio -- yours to use on your next booking or to share with a friend." **Taylor's Mill Follow-up 1B untouched** (flagship-only, as Drew asked) — this deliberately re-diverges 1A/1B, which DREW-88 had synced. Verified by re-reading the saved templates: 1A len 956 (review link + "75% off" + "Powdersville flagship"), 1B len 568 (review link, NO 75% off); empty-template warning absent. Proof: `client/comms/attachments/2026-08-25-followup-1a-review-incentive.png`.
- [x] **SHAHOUD75 coupon — MINTED + LIVE + VERIFIED on prod.** Minted via the dashboard loopback `POST :18794/api/coupons` (`percentOff:75, locationSlug:powdersville, maxUses:1`, id `1fc204b5-...`, location_id 6255578). Propagated to the live booking site through Edge Config: `POST https://whitewallstudios.co/api/validate-coupon {code:SHAHOUD75,location:powdersville}` → `valid:true, percentOff:75`; at `taylors-mill` → `valid:false, "isn't valid for this location"` (flagship scope correct). Percent coupons discount the SESSION line only (add-ons/fees full). One-time use; deactivatable.
- **Framing call (flagged to Drew + soft-FYI Andrew `esc-drew-89-...`, record:ok notify:ok, --reason decision):** wrote the ask as "leave us a review," NOT "leave a 5-star review" — conditioning a discount on the star rating is what Google removes reviews (and can flag a listing) over, and the FTC bars tying the reward to sentiment. Offered Drew his literal "five star" wording as a one-line swap. Money: Drew self-authorizes WWS promos on his own flagship (reversible, session-line only) — no hard gate. Offered to automate the per-customer code minting (generate `<lastname>75` when a review-confirmation email lands).

### Round 128 (thread `1a036c426017a325`, msg `1a03bdd29885f040`) — flagship follow-up subject line + full-copy request (DREW-89 follow-up)
Drew's window active ($30, armed=ON). Follow-up to Round 127 on the same flagship follow-up email: "What's the subject line of that follow up email? Can we make it WhiteWall 75% off for a Google Review. Can you also email me the full copy of that email so I can see it?" Not a repo change — Acuity dashboard (secure.acuityscheduling.com). Same work as DREW-89 (add-msg + comment; stays done). Reply: msg `1a03be32cf5dcd82` (full copy of the email + proof screenshot). Comms: `client/comms/2026-08-25-drew-flagship-followup-subject-and-copy.md`.
- [x] **Flagship Follow-up 1A subject changed to Drew's dictated line — SHIPPED + LIVE + VERIFIED.** Was `%first%, how was your session?` → now **`WhiteWall 75% off for a Google Review`** (verbatim from Drew). Changed in Acuity (`/admin/email-settings/follow-up/1`, the 1A/flagship template) via the persistent Playwright profile + creds; verified by save+reload re-read (subject persisted; body intact, still carries the 75% off + "Powdersville flagship" copy, len 956). Proof: `client/comms/attachments/2026-08-25-followup-1a-subject-and-incentive.png`.
- [x] **Flagship-only scope tightened (found + fixed this round).** The 1A list still grouped **"Taylor's Mill - Six Hours"** with the Powdersville types, so a Taylor's Mill Six Hours booking would have received the flagship 75% off incentive email (and now the "75% off" subject). Dragged that chip from the 1A (flagship) list to the 1B (Taylor's) list (auto-saves on drop). Verified by re-read: **1A = Powdersville types only; 1B = all Taylor's Mill types incl. Six Hours.** The incentive now reaches only flagship clients, exactly Drew's stated intent. Taylor's 1B subject + body untouched (plain thank-you + review request, no discount).
- **No escalation.** Verbatim-dictated subject on Drew's own flagship promo + a reversible flagship-scope tightening — no money/legal/architecture/customer-scale gate. Consistent with the DREW-77/88/89 Acuity email-template fast path (Drew self-authorizes his flagship promo copy).

### Round 129 (thread `1a036c426017a325`, msg `1a03bdf4135e0cdb`) — flagship follow-up: add name to subject + snappier copy (DREW-89 follow-up)
Drew's window active ($30, armed=ON). Follow-up to Round 128 on the same flagship follow-up email, after Drew saw the screenshot: "Yeah let's do my subject line, but add their name in there too. And let's make that email copy a lot more attractive and snappy. Give them an offer quicker, and cut to the chase." He sketched the opener ("NAME, how was your session? We'd love to offer you 75% off your next booking for you, or your friend, if you'd be willing to leave us a google review!"). Not a repo change — Acuity dashboard (flagship Follow-up 1A only). Same work as DREW-89 (add-msg `1a03bdf4135e0cdb` + comment; → done). Reply: msg `1a03bee21ddce450` (new subject + full copy + proof screenshot). Comms: `client/comms/2026-08-25-drew-flagship-followup-copy-rewrite.md`.
- [x] **Subject — name added to Drew's line — SHIPPED + LIVE + VERIFIED.** Was `WhiteWall 75% off for a Google Review` → now **`%first%, WhiteWall 75% off for a Google Review`** (kept Drew's exact wording, prepended the client first name via the `%first%` token). Acuity's own Example preview renders "Laurie, WhiteWall 75% off for a Google Review", confirming the token substitutes per client.
- [x] **Body — rewritten snappy / offer-first per Drew's sketch — SHIPPED + LIVE + VERIFIED.** Opens "%first%, how was your session?", puts the **75% off your next booking** offer in the first two lines, then a tight 3-step how-to (leave a review at the same Google review link `https://g.page/r/CUTS3w9lduxlEAI/review` → email contact@whitewallstudios.co → get your personal 75% off code for the Powdersville flagship, "yours to use or to gift a friend"). **Dropped** the prior "send us your photos to feature on socials" paragraph to cut to the chase (flagged to Drew, offered to restore). Edited via the `<>` HTML source toggle; verified by save + fresh reload re-read of subject and body. Proof: `client/comms/attachments/2026-08-25-followup-1a-name-subject-snappy-copy.png`.
- [x] **Taylor's Mill Follow-up 1B — UNTOUCHED (verified on reload):** subject still `%first%, how was your session?`, body has no 75% off, keeps its own thank-you + review copy. Flagship-only scope preserved.
- **No escalation.** Verbatim-directed copy revision on Drew's own flagship promo, reversible — no money/legal/architecture/customer-scale gate. Same fast path as DREW-77/88/89.

### Round 130 (thread `1a036c426017a325`, msg `1a03bef395134de1`) — Drew sign-off, no work
Drew: "And perfect. Thank you!" — pure sign-off on Round 129 (reply to pip's `1a03bee21ddce450`). Classified approval/fyi; no request, question, or open loop. **No reply sent** (bare thank-you; the Round 129 reply already ended with "Anything else on your list?"). No repo/behavior change. DREW-89 stays done. Logged verbatim: `client/comms/2026-08-25-drew-flagship-followup-copy-rewrite.md` (Round 130 section). last-seen advanced to `1a03bef395134de1` (repo + state).

### Round 131 (thread `1a036c426017a325`, msg `1a03c3c0de349bf0`) — verify Watson's $345 Maegan Lamm refund processed (DREW-86 follow-up)
Drew's window active ($30, armed=ON). Follow-up on DREW-86 (Round 122 Maegan Lamm cancel + rebook investigation) and the Round-122 refund watch: "Pip can you check that refund that Watson was supposed to process? The one for $345. Can you see if it went through? If not, can you push it through and make sure we refund her?" **Read-only verification — no repo/behavior change, no money moved.** Reply: msg `1a03c3f3d3cca2ca`. Comms: `client/comms/2026-08-26-drew-maegan-refund-verification.md`.
- [x] **Refund VERIFIED COMPLETE (two independent reads).** Confirmed against Square `/v2/refunds` directly (authoritative, read-only) AND the dashboard's ingested `payment_refund` row: refund `VQRfmTZHIVdJ6csZw3j0uW0QMAVZY_mucT2Xrb8...` on payment `VQRfmTZHIVdJ6csZw3j0uW0QMAVZY` (booking 1741108891 = ref #2811, Maegan Lamm's canceled Taylor's Mill "Half Day + Lighting Rental, Single Backdrop", Aug 29 9:00am). **Status COMPLETED, $345.00 full amount, destination CARD, reason "Canceled appointment", created 2026-08-25 16:36:57 EDT.** Ingested to the dashboard by the hourly poll at 2026-08-26 00:03:50 EDT.
- **"If not, push it through" branch did NOT apply** — the refund already completed, so there was nothing to move. No money-move gate tripped, no escalation. Watson's refund succeeded; Maegan is fully refunded $345 to her card. DREW-86 → done.

### Round 132 (thread `1a03ee7679c69e27`, msg `1a03ee7679c69e27`) — manual expense entry in the Expense Tracker (DREW-90)
Drew paid $30 (invoice `7b383a5a-6a64-4f97-8637-de4e4e7ebf5f`); window active through 2026-08-27 06:00. New thread. Request: "in the expense tracker, I want to be able to manually input things in there ... but obviously still keep the automations That we already have set up for the cleaning expenses, set up Crew, etc. But I want to be able to manually add things in those different categories as well." **Dashboard-only (booking site untouched); logged here for the paper trail.** Reply: msg `1a03ef2cd7dcb952`. Comms: `client/comms/2026-08-26-drew-expense-tracker-manual-entry.md`.
- [x] **Manual Expenses on the Expense Tracker — SHIPPED + LIVE.** wws-dashboard **PR #159** (merged `f3d980f`, migration `0027` applied to live `wws`, deployed + kickstarted). New pip-owned local `expense_manual_entry` table + `lib/expense-tracker.ts` CRUD (`amountCents`|`amountDollars`, real calendar-date validation, caps) + `/api/expense-tracker/entries` (GET/POST) + `/[id]` (PUT/DELETE). `ManualExpensesSection` on `/expense-tracker`: add/edit/delete owner expenses grouped by category, with the Cleaning Fees (DREW-69) + Setup/Reset Crew (DREW-84) automations preserved above and shown here as locked "Auto" rows with a combined per-category total; Backdrops is now a real manual category (replaced the "coming soon" placeholder), plus free-text custom categories. Writes only local Postgres — Acuity/Square/QBO read-only invariant untouched. NOT an escalation (no money moved, reversible; Drew self-authorizes his own dashboard data). Verified: `npm run build` + seed-mode smoke + full CRUD round-trip on the prod loopback (create → list → edit → delete, DB clean). DREW-90 → done.

### Round 133 (thread `1a03ee7679c69e27`, msg `1a03ef2ce100718b`) — enter backdrop order expenses in the Expense Tracker (DREW-91)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Follow-up to DREW-90: Drew sent "a crap ton of screenshots from backdrop orders ... Ignore [scooters and other non-backdrop items]. You're only looking at things that are specifically backdrops" + a Google Drive folder. **Data-entry task, dashboard-only (booking site untouched); logged here for the paper trail.** Reply: msg `1a03efd9e1f7e58a`. Comms: `client/comms/2026-08-26-drew-backdrop-expense-entry.md`.
- [x] **7 backdrop orders entered in the Expense Tracker Backdrops category — SHIPPED + LIVE.** Read all 10 email screenshots + 5 from the Drive folder, reconstructed 8 distinct Amazon orders, and anchored each on its Order # + subtotal so counts are exact. Excluded the scooters (Razor A5, order 114-0093324) and the mixed-order clutter (smart lock, wall hooks, soap, cards, tape, clamps, router, gateway, order 114-9969706) per Drew. Entered via the live DREW-90 `POST /api/expense-tracker/entries` (Backdrops category, dated each order's placed date, note = provenance + "pre-tax item price"): 114-9969706 (2021-11-22) $193.97 · 114-4416033 (2022-04-02) $199.98 · 114-9049330 (2022-09-22) $99.99 · 114-0093324 (2024-08-30) $479.96 · 114-7443858 (2026-03-20) $919.92 · 111-3879834 (2026-04-17) $459.96 · 114-2619745 (2026-08-26) $299.97. **Total $2,653.75.** Writes only the local `expense_manual_entry` table — Acuity/Square/QBO read-only invariant intact. NOT an escalation (no money moved, reversible; Drew self-authorizes his own dashboard data). Verified: all 7 read back via GET, category totals to $2,653.75, live page 200.
- [~] **Huamei order — FLAGGED, not entered.** IMG_1254 shows 3+ Huamei Seamless Photography Backdrop rolls at $96.98 but the screenshot cut off before the order total, so the full count is unknown. Not entered rather than guess (a silently-wrong ledger figure is worse than a visible gap). Asked Drew for the complete screenshot / to add it to the Drive folder. DREW-91 stays in_progress for this one item.

### Round 134 (thread `1a03ee7679c69e27`, msg `1a03efda170aa95d`) — collapsible Expense Tracker + per-year filter + add-to-fixed categories (DREW-92)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Follow-up on the Expense Tracker: "I need a way to add an expense to even the fixed ones as well, like the Cleaning Fees and the Setup/Reset Crew ... Even just one button. Additionally, let's make everything collapsible. I really should only see the main title of each category ... press a button that uncollapses it and then shows me all the different months ... Ideally, I would be able to filter it per year as well ... same mechanics for the Setup Crew and the Backdrops." **Dashboard-only (booking site untouched); logged here for the paper trail.** Reply: msg `1a03f10dfa279bde`. Comms: `client/comms/2026-08-26-drew-expense-tracker-collapsible-year-filter-manual-fixed.md`.
- [x] **Unified collapsible Expense Tracker — SHIPPED + LIVE.** wws-dashboard **PR #160** (merged `2fbea9c`, deployed + kickstarted). Replaced the three separate sections with ONE unified view fed by a pure, unit-tested `lib/expense-tracker-model.ts` that merges the auto-derived cleaning/crew cost lines (one per triggering booking, $110 / $250 each) and the manual entries into a single category list. `components/expense-tracker/expense-tracker-view.tsx`: (a) every category is a COLLAPSIBLE card, collapsed by default showing only the title + Auto badge + count + total for the selected year (Cleaning Fees reads "10 automated · $1,100", Drew's exact reference), expand for month-by-month with per-month totals; (b) a global YEAR FILTER (All / each year present, default = latest with data), scoping every card — Backdrops spans 2021→2026; (c) an "Add expense" button INSIDE the fixed automated categories too, so an owner-decided extra (e.g. an extra April cleaning) sits next to the automated rows while the automation keeps counting. Writes only the local `expense_manual_entry` table — Acuity/Square/QBO read-only invariant intact; NO money-figure change (manual extras stay display-only, not wired into Net to Our Bank). NOT an escalation (reversible; Drew self-authorizes his own dashboard data). Verified: `npm run build` + 323 unit tests (7 new in `expense-tracker-model.test.ts`); live-DB Playwright on a non-prod port (collapsed default, expand Cleaning Fees [auto rows] + Backdrops [$2,653.75 across 2021-2026], year filter, descriptions wrap without colliding with Amount); CRUD round-trip adding an extra cleaning to the fixed Cleaning Fees category, then deleted (DB clean); prod loopback :18794 200 + markers present. DREW-92 → done.
- [~] **Backdrops backlog (DREW-91) — still open on the one Huamei order.** Drew nudged "let me know whenever all the backlog is in there." The 7 orders remain in ($2,653.75); only the Huamei rolls ($96.98) are outstanding (screenshot cut off before the total). Told Drew again it lands the moment he drops the full shot in the Drive folder. DREW-91 stays in_progress.

### Round 135 (thread `1a03ee7679c69e27`, msg `1a03fa957c6d09fb`) — more backdrop order screenshots from Max in the Drive folder (DREW-91 round 2)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Drew: "I just added some more screenshots that Mac sent over ... Can you go back and find out which ones are new and then account for those in there as well?" **Dashboard-only data entry (booking site untouched); logged here for the paper trail.** Reply: msg `1a03fb041a69f143`. Comms: `client/comms/2026-08-26-drew-backdrop-expense-entry-round2.md`.
- [x] **4 new backdrop orders entered — LIVE.** Re-downloaded the Drive folder, diffed against the DREW-91 round-1 pull → 7 new screenshots (all `1.4x/1.5x PM`), deduped by Amazon/B&H order number. Entered 4 anchorable orders (backdrops only, pre-tax item price where shown) via the live DREW-90 `POST :18794/api/expense-tracker/entries` (loopback; no code change, reversible local `expense_manual_entry` table): Amazon 112-9716469 (1x Kate Seamless Black, $99.99, 2025-11-07); Amazon 112-6158566 (5x HUAMEIZOOM white @ $98.99, $494.95, 2025-06-26, $18.99 tether lock excluded); B&H 915503828 (16x Savage seamless @ $76.99, $1,231.84, 2026-01-13); B&H 1085873219 (1x Savage Widetone Pecan, $139.78 order total, 2022-08-22). **+$1,966.56; Backdrops category now $4,620.31** (verified via loopback GET, 11 entries reconcile). NOT an escalation (Drew self-authorizes his own data). Skipped 1 duplicate (114-2619745, already in — that shot only showed the tax-inclusive total).
- [~] **DREW-91 stays in_progress — 2 items still need Drew.** (1) Two screenshots were Amazon "Buy it again" browse-list views with no order numbers or prices (2022-era Savage colors + a Huamei black) — can't anchor a total without the itemized order receipt; asked Drew to screenshot those specific order summaries. (2) The old cut-off $96.98 Huamei order (round 1, IMG_1254) still lacks its total; the new June-2025 HUAMEIZOOM order is $98.99/roll so likely separate — asked Drew to confirm (avoid double-count) or send the full receipt.

### Round 136 (thread `1a03ee7679c69e27`, msg `1a03fae66d7e1085`) — full audit of the entire Drive folder for backdrop orders (DREW-91 round 3)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Drew: "I'm also pretty sure there's a whole lot more than just seven orders ... check that Google Drive folder again ... audit that entire Google Drive folder and all the photos in there, not just the photos I sent you in the email ... There should be over 35 of them." **Dashboard-only data entry (booking site untouched); logged here for the paper trail.** Reply: msg `1a03fbfcc333c01a`. Comms: `client/comms/2026-08-26-drew-backdrop-expense-entry-round3-full-audit.md`.
- [x] **Full folder audit + 10 more backdrop orders entered — LIVE.** Read every one of the 40 Drive photos (Drive-only per Drew; 5 parallel image extractors). Prior rounds had only covered IMG_1246-1254 / IMG_1276-1280 / the PM screenshots; the never-audited **IMG_1256-1275** range held the bulk. Entered 10 more anchorable orders (backdrops only, pre-tax subtotal, dated to when placed) via the live DREW-90 `POST :18794/api/expense-tracker/entries` (loopback; no code change, reversible local `expense_manual_entry` table): 114-4185400 (Sep-22-2022, $99.99); 114-7202693 (Apr-15-2023, $219.98); 114-0918520 (Jul-25-2023, $109.99); 114-6498757 (Jul-25-2023, $329.97); 114-9186508 (Aug-29-2023, $879.92); 114-8581413 (Jan-04-2024, $109.99); 114-4221932 (Jan-04-2024, $452.85); 114-5052536 (Jan-04-2024, $879.92); 114-7122205 (Feb-28-2024, $439.96); 114-9586693 (Apr-25-2024, $387.92, 4x Huamei @ $96.98). **+$3,910.49; Backdrops category now $8,530.80 across 21 orders** (reconciles $4,620.31 + $3,910.49; verified via loopback GET). Per-year: 2021 $193.97 / 2022 $539.74 / 2023 $1,539.86 / 2024 $2,750.60 / 2025 $594.94 / 2026 $2,911.69. NOT an escalation (Drew self-authorizes his own data).
- [x] **Resolved: the old cut-off $96.98 Huamei order** is now anchored (IMG_1256 is its summary: order 114-9586693, Apr 25 2024, $387.92 = 4x $96.98). Distinct from the Jun-2025 HUAMEIZOOM $98.99 order already in; entered once, no double-count.
- [x] **Confirmed 2025 is covered** (2 orders, $594.94 — both from round 2). Drew's "nothing in 2025" was a pre-Round-135 tab; his 16:06 message predated the 16:08 reply that added them.
- [~] **DREW-91 stays in_progress — 2 items still need Drew.** (1) One Mar-13-2023 order (2x Savage, $219.98) shows a FULL refund on its screenshot (Refund Total $233.18) + a "Replacement ordered" banner — excluded as not-money-spent; asked Drew whether a paid replacement shipped. (2) The two "Buy it again" browse-list screenshots (Jul/Nov 2022 Savage colors + an Aug 2022 Huamei black) have no order number or price — asked Drew for the itemized order summaries.

### Round 137 (thread `1a03ee7679c69e27`, msg `1a03fdbc854e1ec5`) — owner-only 8-Hour Override in the Session Builder (DREW-93)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Drew: "we dont allow the 8 hour session to book earlier than 12:30pm. Thats fine by default, but I want the ability to override that when im building the session out in the session builder ... a small button ... 8-Hour Override ... next to the 'Available Times For X day' ... only in the session builder for me as the owner ... Not on the site. And ... make sure it reflects in the schedule properly too." → **DREW-93**. Ack: msg `1a03fe0639cf27ae`; confirmation: msg `1a03ff3c515d5e64`. Comms: `client/comms/2026-08-26-drew-8hour-override.md`. Spans BOTH repos.
- [x] **8-Hour Override pill in the Session Builder — SHIPPED + LIVE (both repos).** Booking **PR #170** (merged `db345da`, Vercel prod-deployed) + dashboard **PR #161** (merged `7fb655c`, deployed + kickstarted on the mini). A small "8-Hour Override" pill renders next to "Available times for [day]" **only in BUILDER mode** for a duration with an earliest-start floor (today the 8h Flagship). Off by default (12:30pm floor holds exactly as today); tap it and the pre-floor slots appear so the owner can start the 8h session before 12:30. It also shows in the empty-slots state so the owner can reveal hidden early starts. **Never renders on the customer site.**
- [x] **Reflects in the schedule (the early start carries through) via the SIGNED offer.** The override is signed per-session into the HMAC minted link (`lib/session-links.ts` `buildOfferPayload`, keyed by BOOKING_SECRET). The booking site lifts the 12:30 floor at all four enforcement sites ONLY for a signature-verified offer carrying the flag: `booking-flow.js` client filter, `availability-times.js` (display via `earlyStartOverride=1`), `create-checkout.js` reject (`offer && s.earlyStartOverride`), `_lib/cart.js` computeCart reject. A raw client POST has `offer=null`, so an injected flag is ignored and the floor still holds — the override is structurally impossible to trigger without a dashboard signature.
- [x] **Latent staleness fixed as a side effect.** The embedded builder copy (`public/session-builder-app/*`) had not been re-synced since 2026-07-29, so the dashboard re-sync also pulled in already-live prod content (DREW-80 add-on placement removal, DREW-83 crew card, pv-8 subtext, booking.css). Prod content, not new behavior.
- [x] **Verified.** `npm run build` (both) + `node --check` all touched JS; **13 assertions against the real `_lib/acuity.js` + `_lib/cart.js`** (floor predicate; availability filter; the create-checkout gate — raw-POST-injected flag STILL rejected, verified-offer bypasses; computeCart rejects/accepts); a **Playwright drive of the real builder page** (pill appears, floor holds until toggled, request gains `earlyStartOverride=1`, early slots appear, customer mode has no pill/no early slots). Live: prod `availability-times` returns pre-12:30 starts only with the param (00:00 vs 12:30); dashboard proxy passes it through; agent surface 401 (AGENT_API_KEYS intact — deploy trap avoided).

### Round 138 (thread `1a03ee7679c69e27`, msg `1a03fe4b4f628106`) — Session Builder "Export Summary" button (DREW-94)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Drew: "Another thing in th session builder to build next: Export Summary button. Once I save the session, I can either get the link via that button, or I can export a pdf of the summary as to what is included in the session. The date, time, full financial break down, duration, add-ons, etc. then also put the link directly in the summary too so they could just tap it and go straight to actually booking from there. The pdf should always be named the Session Name x WhiteWall Custom Offer ... I click that button, then it immediately just downloads and goes straight into my downloads folder. I should also be able to press that button as a pill at the bottom of the save session." → **DREW-94**. Ack: msg `1a03ff9ce62f60f3`; confirmation: msg `1a04006bb85b09be`. Comms: `client/comms/2026-08-26-drew-session-builder-export-summary.md`. Dashboard-only (wws-dashboard).
- [x] **Export Summary pill in the Session Builder — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #162** (merged `e34449b`, deployed + kickstarted on the mini). A new "Export Summary" pill on every saved-session card (bottom action row, next to Get Link). One tap → downloads a PDF straight to the downloads folder (jsPDF `save()`, no dialog), named **`{Session Name} x WhiteWall Custom Offer.pdf`** (Drew's example → `Shahoud Baby Shower x WhiteWall Custom Offer.pdf`). Owner-only tool; never on the customer site.
- [x] **PDF content = the full summary.** Session name, type, location + address, date/time, duration, **per-day add-on line items**, and the **full financial breakdown** (sessions, add-ons, add-on savings, cleaning fee, multi-day discount, ownership adjustments, total). A tappable **"Book this session"** button + the raw link are embedded, so a recipient goes straight to booking.
- [x] **Money is server-authoritative.** New `POST /api/session-drafts/[id]/summary` loads the draft, mints the live locked link (same recompute+sign+activate path as Get Link), and returns the summary from `computeFlowV2Totals` — no money math in the browser. `lib/session-builder/session-summary.ts` (`buildSessionSummary`) reuses the exact pricing path the customer link is signed from → PDF == quote == charge by construction. `lib/session-builder/export-summary-pdf.ts` is the client-only jsPDF renderer, dynamic-imported so jsPDF stays out of the main bundle; a WinAnsi-safe sanitizer maps the multi-day check-in/out arrow to an en-dash. Reads `session_draft` + writes only our Edge Config (via `mintSessionLink`) — Acuity/Square/QBO read-only invariant untouched.
- [x] **Verified.** `npm run build` (exit 0); **326 unit tests** (3 new: filename convention, flow-v2 itemization, pre-flow-v2 degrade); reconciled `buildSessionSummary.finalTotalCents` against stored `final_total_cents` for **all 4 live flow-v2 drafts → 0 mismatch** (incl. a real 3-day event, 9 add-on lines, $2,910 exact); rendered + eyeballed the real single-day and 3-day PDFs. Live: **Playwright drive of the deployed Session Builder** clicked the pill and confirmed a real download named `Sundra Baby Shower x WhiteWall Custom Offer.pdf` (valid PDF); prod summary route returns 200 with a real minted booking URL; agent surface 401 (AGENT_API_KEYS intact — deploy trap avoided).

### Round 139 (thread `1a03ee7679c69e27`, msg `1a0400c0e4f7b652`) — Export Summary PDF redesign: real logo + studio photo + contact block (DREW-94 follow-up)
Drew's window active (Drew's $30, through 2026-08-27 06:00). Drew: "Incredible, it worked out perfectly. Replace the WhiteWall Studios title at the top of the PDF with our actual logo, including the asset in this email ... slide all the content underneath ... down, creating that same white dead space ... I want to add in a photo. The photo is included in this email as well. Also, make it more clear: the link to the website, the email, my phone number, which is 803-873-8153 ... "Have any questions? Reach out to us," and then provide all three of those links. Also, make the "Book this session" actually say "Tap to book this session." ... if you think it makes more sense to make the whole top third/half the beautiful photo ... and then put the logo on top of that, that's also fine with me too ... I'll let you make that design choice." (attachments: the black `whitewall studios & events` logo + a studio interior photo). Ack: msg `1a04010bedad01d3`; confirmation (with the real sample PDF attached): msg `1a0401c25ba4445f`. Comms: `client/comms/2026-08-26-drew-export-summary-logo-photo-redesign.md`. Dashboard-only (wws-dashboard); owner-only PDF; NO escalation (no money/architecture/legal/scale; reversible).
- [x] **Export Summary PDF redesign — SHIPPED + LIVE + VERIFIED.** Dashboard **PR #163** (merged `5400428`, deployed + kickstarted on the mini). Took Drew's greenlit hero option: the top of the PDF is now a **full-bleed studio photo** with a 44% dark scrim and **his real logo (recolored white) laid over it**, plus a "CUSTOM SESSION OFFER" overline — replacing the old "WHITEWALL STUDIOS" text masthead. Breathing room before the session content. New **"Have any questions? Reach out to us." contact block** with website / email / phone (803-873-8153), each a tappable link. Book button copy **"Book this session" → "Tap to book this session."**
- [x] **Assets + weight.** New `lib/session-builder/export-summary-assets.ts` — the white logo + the photo cropped to the 612×232 header band (JPEG q82), embedded as base64 data URLs. The jsPDF doc is now built with `compress:true` so the mostly-transparent logo stream deflates — a full sample PDF is ~115 KB (down from ~3.7 MB before compression). Section spacing tightened so a standard single-day session still fits on ONE page (multi-day flows to page 2 as before). **All money math unchanged** — still the server-authoritative `buildSessionSummary` / `computeFlowV2Totals`; this only lays it out.
- [x] **Verified.** `npm run build` (exit 0) + **326 unit tests** pass; rendered single-day (1 page) + 3-day event (2 pages) PDFs headless and eyeballed; **live Playwright drive of the DEPLOYED builder** → real download `Sundra Baby Shower x WhiteWall Custom Offer.pdf` (1 page, 115 KB), text-verified (button renamed, contact block + phone/email/website present, old text masthead gone → now an image, 3 embedded image streams, real booking link) and eyeballed; page 200 / agent 401 / Watson 400 (AGENT_API_KEYS intact — deploy trap avoided). Proof: `client/comms/attachments/2026-08-26-export-summary-logo-photo-redesign-live.png`. **DREW-94 → done.**

### Round 141 (thread `1a043e7152dd0429`, msg `1a043ee8ecf93848`) — Event booking got the wrong location's email; root cause + permanent-fix verification (DREW-95)
Drew's window active (Drew's $30, through 2026-08-28 06:00). Drew: an event client (Denise) "got all her booking information for the Taylor's Mill location, not the Powdersville location ... wrong information and door codes ... Can you go into Acuity ... verify that this won't happen again ... figure out why this happened ... how we could permanently fix that ... especially for event bookings or ... the Powdersville/Taylor's Mill location?" (also: Maggie booked 12am vs 12pm — FYI only, Drew already fixed it). → **DREW-95**. Reply: msg `1a044049326c45bd`. Comms: `client/comms/2026-08-27-drew-denise-event-location-email-misroute.md`. Acuity-config + verification round; NO booking-site behavior change (doc-only note in `api/_lib/acuity.js`).
- [x] **Root cause found.** Denise Ko's booking is a Powdersville 8-Hour EVENT (Acuity id 1742001484, type 94823049, on the Powdersville calendar 6255578, Aug 29 12:30pm) — the booking is correct; only the email was wrong. She booked **2026-07-22**, before the **DREW-77 (2026-08-25)** fix that moved the 8-Hour Powdersville type off the Taylor's Mill confirmation + reminder templates. At her booking time the 8-Hour was still on the Taylor's templates, so her confirmation carried Taylor's info (250 Mill St; door codes 7530/2319/508).
- [x] **Verified permanently fixed (Acuity dashboard, checkbox-by-checkbox).** Booking Confirmation (Taylor's) = exactly the 6 Taylor's types, no Powdersville. Booking Confirmation 2 (Powdersville) = exactly the 7 Powdersville types **including the 8-Hour**, no Taylor's. Reminder 1A = the 6 Taylor's types; Reminder 1B = the 7 Powdersville types incl. the 8-Hour. Zero cross-assignment. Cancellation + Reschedule are single global "All appointments" templates (no location codes → cannot misroute). Denise's Aug 28 reminder will now send Powdersville info on its own.
- [x] **Blast radius scoped.** Acuity API query for type 94823049 from today forward returns exactly 1 upcoming booking (Denise's). No other pre-fix 8-Hour Powdersville booking is still upcoming, so she is the only affected customer.
- [x] **Permanent safeguard documented.** The type → email-template assignment is Acuity-dashboard-only (no API, not code-enforceable). Added the routing INVARIANT + incident history as a comment block next to the appointment-type allowlist in `api/_lib/acuity.js`, so any newly created appointment type gets attached to its location's confirmation + reminder templates and re-verified. `node --check` clean.
- [x] **Denise given correct Powdersville info.** Handed Drew the exact Powdersville block to relay (he is live-texting her): address 2709 Powdersville Rd Easley SC 29642 (real #2699), **Studio Door Code 1923** (her wrong email said 2319, which is actually the PV storage lockbox), Storage Building Lockbox 2319 (chairs/tables/PA/TV for her event), and all Powdersville video links (studio access, storage/equipment, chair, lighting, backdrop set-up/tear-down, reset walkthrough). Also offered to resend the formatted confirmation from the White Wall system to denise@bloominmama.com on his go.
- [x] **No escalation.** No money, no architecture, no legal/policy copy, no at-scale customer send (single customer; info relayed via Drew). Acuity email-routing verification is established Foreman scope (DREW-77 precedent).


### Round 142 (thread `1a043e7152dd0429`, msg `1a0440a28fee7092`) — resent Denise's Powdersville confirmation from the White Wall system (DREW-95 follow-up)
Drew's window active (Drew's $30, through 2026-08-28 06:00). Drew greenlit the DREW-95 standing offer: "Perfect. Yes please send it to her with the correct info like normal. Let me know once its done." Reply: msg `1a0442584047594d`. Comms: `client/comms/2026-08-27-drew-denise-event-location-email-misroute.md`. Acuity action only — NO booking-site code change, NO escalation (single-customer corrective send, Drew-directed).
- [x] **Resent Denise Ko's booking confirmation from the White Wall system (Acuity) to denise@bloominmama.com.** Pre-send re-verified in Acuity Client Emails that **Booking Confirmation 2** (Powdersville template) has `Powdersville Studio - Eight Hours` **checked** and all 6 Taylor's Mill types **unchecked**, so the resend regenerates correct Powdersville info (2709 Powdersville Rd, Studio Door Code 1923, storage lockbox 2319, PV video links). Acuity returned the success banner "Appointment confirmation e-mail for appointment on August 29, 2026 12:30pm resent to denise@bloominmama.com." **DREW-95 → done.**
### Round 143 (thread `1a049294d382889a`, msg `1a049294d382889a`) — change the Google review incentive from 75% off to a flat $100 off (DREW-89, reopened)
Drew's window active (Drew's $30, invoice `54a3d181...`, through 2026-08-29 06:00, armed=ON). Drew (after talking with Max): "if we actually locked in the 75% language, we need to change that. We want to make it a flat $100 off their next booking, whether it's an event or a session ... they need to send us proof that they left a Google review ... 75% works ... [for] a one- or two-hour session, but if it's a full-blown event ... that definitely won't work. ... Give me a rundown of exactly what we have ... and a proposal as to what you want to change the subject line and email copy to." → **DREW-89 reopened**. Ack: msg `1a0492f8ee742f84`; rundown + proposal: msg `1a04931af96ab641`. Comms: `client/comms/2026-08-28-drew-review-incentive-100-off-proposal.md`. **Proposal round — nothing flipped live yet (Drew asked to review the copy first); ship pending his go.**
- [x] **Rundown of current state delivered.** Where it lives (Acuity flagship Follow-up 1A, Powdersville only, one day post-session; Taylor's Mill has no incentive), the exact live subject (`%first%, WhiteWall 75% off for a Google Review`), the exact live body (verbatim in the comms), and how the code works (per-customer `<lastname>75` percent code minted on emailed proof; percent hits the session line only).
- [x] **Proposal delivered: flat $100 off.** Switch the code from a percent (capped 1..99, session-line only) to a flat `amountOffCents` code that comes off the whole order (session + add-ons) — $100 is $100 on a short session or a full event, no runaway discount. Codes become `<lastname>100`. Proposed new subject + new body copy (both in the comms), keeping the snappy offer-first structure Drew liked.
- [x] **Testable demo minted + prod-verified.** `SHAHOUD100` (flat $100, Powdersville, one use; id `0ba0347c-ff49-4c45-895d-a6603aff629f`) minted via loopback and confirmed on the live site: `validate-coupon` returns `$100 off` at Powdersville, rejected at Taylor's Mill. Drew can test it in the flagship checkout now.
- [x] **Two decisions surfaced to Drew.** (1) keep flagship-only (events are Powdersville anyway) vs also run $100 on Taylor's; (2) kept "leave us a review" over "5 star review" for review-gating compliance (Google/FTC), offered his exact words.
- [x] **Ship (SHIPPED Round 144 on Drew's go):** swapped Acuity Follow-up 1A + 1B to the $100 version + retired the live 75% code (SHAHOUD75) + mint `<lastname>100` going forward. See Round 144.
- [x] **No hard escalation** — Drew's own flagship promo, reversible, no company money. Soft-FYI recorded for Andrew (Foreman-authored customer-facing incentive copy, compliance nuance; same class as Round 127).

### Round 144 (thread `1a049294d382889a`, msg `1a04933ce7f57eeb`) — SHIP the flat $100 review incentive at BOTH locations (DREW-89)
Drew's window active (Drew's $30, invoice `54a3d181...`, through 2026-08-29 06:00, armed=ON). Drew, approving the Round 143 proposal: "Let's go ahead and fix it all to be the $100 option, and we can make it for both locations. It doesn't matter if it's Taylor's Mill or Powdersville. We want them on both, regardless." → **DREW-89 shipped + LIVE + verified.** Reply: msg `1a04944373c9b758`. Comms: `client/comms/2026-08-28-drew-review-incentive-100-off-approved-both-locations.md`. Acuity dashboard + live coupons only — NO booking-site repo/code change.
- [x] **Acuity Follow-up emails swapped to the flat $100 copy, BOTH locations.** Follow-up 1A (Powdersville) AND 1B (Taylor's Mill) now carry the identical incentive: subject `%first%, $100 off your next booking for a Google review`; body offers **$100 off your next booking, session or full event**, keeps the 3-step how-to (Google review link `https://g.page/r/CUTS3w9lduxlEAI/review` → email contact@whitewallstudios.co → personal $100 code). Powdersville-flagship-only wording dropped (now location-neutral). Edited via the Acuity Redactor `<>` source toggle; **verified on fresh reload** — both templates: $100 present, 75% gone, subject updated, review link + contact email intact.
- [x] **1B previously had a plain thank-you + "send us your photos for our socials" ask; replaced with the $100 offer** so both locations match. Flagged the socials-ask removal to Drew, offered to add it back.
- [x] **Coupon mechanic → flat $100, company-wide (both studios).** `SHAHOUD75` deactivated (PUT active:false, record + redemption history preserved). `SHAHOUD100` → `locationSlug:"any"` (location_id null = both studios), flat `amountOffCents:10000` off the WHOLE order (session + add-ons). **Prod-verified:** `validate-coupon` returns `$100 off` at Powdersville AND Taylor's Mill; SHAHOUD75 now rejected. Going-forward per-customer codes = `<lastname>100` (amountOffCents 10000, locationSlug any, maxUses 1).
- [x] **Review-gating call kept:** wording stays "leave us a review" not "leave a 5 star review" (Google pulls reviews tied to a star rating; FTC bars tying reward to sentiment). Offered Drew his exact "5 star" wording as a one-line swap. Open call (2) from Round 143 still Drew's to make.
- [x] **No hard escalation** — Drew's own promo, reversible, no company money; Drew self-authorizes WWS promo/money. Soft-FYI recorded for Andrew (`esc-ww-review-incentive-now-flat-100-off-live-at-both-locations-decision`, record:ok notify:ok).
