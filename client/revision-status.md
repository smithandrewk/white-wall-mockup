...
- [x] **Item 4a/4b/4c — crew add-on mechanics overhaul, booking PR (DREW-80) — BUILT + UNIT-VERIFIED (no real sends).** Build plan: `client/comms/2026-08-25-drew-crew-mechanics-build-plan.md`. **Booking PR** ships all three coupled (no info-loss gap):
...
- [ ] **Item 4d — dashboard crew-notes box (DREW-80) — NEXT (separate dashboard PR).** On `/bookings/[id]`: render the crew add-on LAST + an autosave notes textarea tied to the booking (new `booking_crew_notes` table/migration; our own data, inside the read-only-upstreams invariant).
