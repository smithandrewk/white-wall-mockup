...
- **Dark launch: `COUPONS` is intentionally left UNSET.** The promo field ships visible but every code is rejected until codes are configured. No real codes were created. UI + server are isolated so they can never break a normal no-coupon booking.
